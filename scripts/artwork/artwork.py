...
def icon(kind, scale=1):
    ...
    if kind in ('grid','window','focus'):
        ...
        p+=path('M-135 105 H137',INK,2)
        for x,y,r in [(-95,58,22),(-16,29,39),(89,-17,61)]:
            p+=circle(x,y,r,'#FCFAF4',INK,2)
            for angle in range(0,360,60):
                a=math.radians(angle);xx=x+math.cos(a)*r*.64;yy=y+math.sin(a)*r*.64
                p+=path(f'M{x} {y} L{xx} {yy}',ACCENT,2)+circle(xx,yy,4,ACCENT,'none')
            p+=circle(x,y,6,ACCENT,'none')
    elif kind=='microphone':
        p+=rect(-38,-99,76,123,'#FCFAF4',INK,37)
        for y in (-75,-55,-35,-15):p+=path(f'M-19 {y} H19',ACCENT,4)
        p+=path('M-69 -4 V10 C-69 88 69 88 69 10 V-4',INK,6)
        p+=path('M0 70 V109 M-42 109 H42',INK,6)
    elif kind=='activity':
        p+=path('M-128 -87 V92',INK,2)
        for y,w in [(-60,134),(-9,191),(42,91)]:
            p+=path(f'M-107 {y+11} H111',LINE,18)
            p+=rect(-107,y,w,22,ACCENT,radius=5)
    elif kind=='cycle':
        p+=path('M-77 -30 C-61 -102 72 -104 89 -18',ACCENT,8)
        p+=poly([(66,-32),(106,-31),(86,4)],ACCENT)
        p+=path('M77 31 C60 104 -72 103 -89 18',INK,8)
        p+=poly([(-65,33),(-106,32),(-85,-4)],INK)
    elif kind=='scope':
...
def overview(c):
    ...
    p+=path('M236 309 H1205',ACCENT,3)
